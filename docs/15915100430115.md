...
```
这里有个需要注意也是官方文档上提到的一点，如果我们调用dispatch_barrier_async时将Barrier blocks提交到一个global queue(没有任务入队的过程，会立即执行)和串行队列，barrier blocks执行效果与dispatch_async()一致；将Barrier blocks提交到使用dispatch_queue_create和DISPATCH_QUEUE_CONCURRENT属性创建的并行queue时，它才会表现的如同预期。详细原因见后续源码分析。
...
```
_dispatch_get_root_queue从_dispatch_root_queues结构体中获取对应优先级的队列。最后1bit为1的代表overcommit，带有overcommit标记的队列会在任务提交时强制新建一个线程处理它。注意这里的强制，是指不考虑系统当前状态，即使线程池足够，也仍然新建线程，而不是从线程池里取一个。
...
_dispatch_root_queues取出的dispatch_queue_s队列的do_ctxt字段，用来存储线程池相关数据，比如用于线程挂起和唤醒的信号量、线程池尺寸等，定义于_dispatch_root_queue_contexts结构体中，每个线程池的最大线程数限制是255。_dispatch_root_queue_contexts结构体数组的部分源码：
```
...
如果我们调用dispatch_barrier_async时将Barrier blocks提交到一个global queue，barrier blocks执行效果与dispatch_async()一致；只有将Barrier blocks提交到使用dispatch_queue_create和DISPATCH_QUEUE_CONCURRENT属性创建的queue时它才会表现的如同预期。barrier体现在任务分派的时候，async体现在任务提交的时候。
