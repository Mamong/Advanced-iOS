...
现在在重新声明一个 func_c 函数，并且对这个 x 进行加 1 的操作。你觉得会打印什么结果？![](media/16339519643172/16345335277434.jpg)
...
## \_\_slots\_\_
类方法又可细分为实例方法、静态方法和类方法，Python 语言允许为类动态地添加这 3 种方法；但对于实例对象，则只允许动态地添加实例方法，不能添加类方法和静态方法。
...
此外，``__slots__`` 属性对由该类派生出来的子类，也是不起作用的。如果为子类也设置有 ``__slots__`` 属性，那么子类实例对象允许动态添加的属性和方法，是子类中 ``__slots__`` 属性和父类 ``__slots__`` 属性的和。
...
1. 第一种语法格式用来查看某个变量（类对象）的具体类型，obj 表示某个变量或者类对象。
2. 第二种语法格式用来创建类，其中 name 表示类的名称；bases 表示一个元组，其中存储的是该类的父类；dict 表示一个字典，用于表示类内定义的属性或者方法。
...
Python 枚举类中各个成员必须保证 name 互不相同，但 value 可以相同。Python 允许这种情况的发生，它会将 green 当做是 red 的别名，因此当访问 green 成员时，最终输出的是 red。
...
除了通过继承 Enum 类的方法创建枚举类，还可以使用 Enum() 函数创建枚举类。 Enum() 函数可接受 2 个参数，第一个用于指定枚举类的类名，第二个参数用于指定枚举类中的多个成员。
```
from enum import Enum
#创建一个枚举类
...
增加了标准库pathlib，用于按面向对象的方式操作文件路径
增加了标准库enum，用于定义枚举类。

Python3.5
扩展了迭代拆包运算符 * 、字典拆包运算符 ** 的用法，可以在元组、列表、集合、字典表达式中使用；可以同时使用多次
允许使用Python2风格的 % 格式化字符串
增加了函数注释的语法，用于说明形参、返回值的类型。
增加了用async、await关键字定义协程的语法：

python 3.6
可以在数字中插入下划线作为分隔符，提高可读性。
Python格式化字符串f-string概览
定义元类的更好方法：给类定义__init_subclass__()方法，用于初始化子类。
...
增加了赋值表达式的语法，可以给表达式中的变量赋值
定义函数时，在正斜杆 / 之前的参数都会被视作位置参数
可以在f字符串中用变量=的形式打印变量的值
可以在finally语句块中使用continue关键字。
multiprocessing模块增加了一个SharedMemory类，用于创建进程之间的共享内存。


Python中list与collections.abc.Sequence是什么关系？
https://www.zhihu.com/question/39383097?sort=created